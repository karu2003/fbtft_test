...
class Draw:
	...
	def rect(self, color, rect, width=0):
		if (width==0):
			for y in range(int(rect.top), int(rect.top) + int(rect.height)):
				self.fb.hline(int(rect.left), y, int(rect.width), color)
		else:
...
#		elif (self.bits_per_pixel == 16):
	...
	def fill(self, color):
		if (self.bits_per_pixel == 1):
		    ...
		    #colstr = struct.pack("H", color)
		    buf = np.full(int(self.screensize/4), color,dtype=np.int)
		    self.fbp.seek(0)
		    self.fbp.write(buf)
		else:
	...
	def putstr(self, x, y, str, color, size=1):
		...
		if (type(str) == bytes):
			for i, c in enumerate(str):
				(x, y) = self.putchar(x, y, c, color, size)
		else:		
			for c in str:
